...
pub fn mallory_second_measurement(
    ...
    let first_measurement_ref = entangled_pair
    ...
    let basis: Basis = Basis::get_random_basis(side);
    let first_basis = first_measurement_ref.get_basis()?;
    let first_value = first_measurement_ref.get_value()?;
    let first_node_id = first_measurement_ref.node_id;

    let basis_diff = calculate_basis_difference(basis, first_basis);
    ...
    let first_node = nodes
        .get_mut(&first_node_id)
        .ok_or(PairError::PairNotFound(first_node_id))?;
