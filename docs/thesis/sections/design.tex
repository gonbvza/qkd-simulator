...
This channel is a multiple-writer single-reader channel. Instead of sharing state between both layers, which would require locking due to Rust's mutable ownership rules, the entry point thread send event requests that are received by the main loop.
